    ...
    <main>
      <section className="relative py-24 md:py-36 overflow-hidden">
        ...
        <div className="container relative z-10 text-center space-y-6 animate-fade-up">
          ...
          <a href="#products" className="inline-block bg-primary text-primary-foreground font-bold px-8 py-3.5 rounded-lg hover:opacity-90 transition-opacity active:scale-95">
            Shop Now
          </a>
        </div>
      </section>
          ...
          {categories.map((cat) => (
            <button key={cat} onClick={() => setActive(cat)}
              className={`px-5 py-2 rounded-full text-sm font-medium transition-colors whitespace-nowrap active:scale-95 ${
                active === cat ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-border"
              }`}>
              {cat}
            </button>
          ))}
        </div>
        {error && <p className="text-xs text-amber-500 text-center">{error}</p>}
        {loading ? (
